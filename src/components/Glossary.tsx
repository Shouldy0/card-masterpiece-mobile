...
const GLOSSARY: GlossaryTerm[] = [
  {
    ...
    impact:
      "Determina quante azioni puoi compiere in un turno. La Lucidità aumenta di 1 ogni turno, fino a un massimo di 6.",
  },
  {
    id: "trauma",
    name: "Trauma",
    ...
    example:
      "Se giochi troppe carte in un territorio 'Trauma', la tua Lucidità massima potrebbe ridursi.",
    impact:
      "Un alto livello di Trauma danneggia direttamente la tua Chiarezza (HP) alla fine del turno.",
  },
  {
    id: "archetype",
    name: "Archetipo",
  ...
  },
  {
    ...
    definition:
      "Difese sociali usate per nascondere la propria vulnerabilità e indebolire l'altro.",
  ...
  },
  {
    ...
    impact:
      "Hanno effetti unici e spesso imprevedibili che possono ribaltare le sorti di un match.",
  },
  {
    id: "territories",
    name: "Territori",
    ...
    impact:
      "Ogni territorio ha regole proprie che influenzano il potere delle carte giocate al suo interno.",
  },
];
                  ...
                  {GLOSSARY.map((term) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedId === term.id
                          ? "bg-gold/10 border border-gold/30 scale-[0.98]"
                          : "hover:bg-white/5",
                      )}
                    >
                      <term.icon
                      ...
                      <span
                        className={cn(
                          ...
                          selectedId === term.id ? "text-gold" : "text-white/30",
                        )}
                      >
                        {term.name}
                      </span>
                    ...
                    {selectedTerm ? (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div
                            className={cn(
                              "size-14 rounded-2xl bg-white/5 flex items-center justify-center border border-white/10",
                              selectedTerm.color,
                            )}
                          >
                            <selectedTerm.icon className="size-8" />
                          </div>
                          <h3 className="font-display text-2xl text-white font-bold uppercase tracking-widest">
                            {selectedTerm.name}
                          </h3>
                        </div>

                        <section className="space-y-3">
                          <h4 className="text-[10px] font-black text-gold uppercase tracking-[0.3em]">
                            Definizione
                          </h4>
                      ...
                      <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                        ...
                        <p className="text-xs uppercase tracking-widest">
                          Seleziona un termine per approfondire la conoscenza della Reverie
                        </p>
                      </div>
                    )}
                  </AnimatePresence>
