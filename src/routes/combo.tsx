...
const UI_THEME = {
  ...
  premium_btn:
    "rounded-2xl font-display text-xs uppercase tracking-[0.25em] transition-all active:scale-95",
...
const getCatMeta = (variant: any) => ({
  character: {
    label: variant.theme.labels.character,
    color: "text-mystic-glow",
    gradient: "from-mystic/40 to-abyss",
    bgGlow: "oklch(0.55 0.22 295 / 40%)",
  },
  setting: {
    label: variant.theme.labels.setting,
    color: "text-azure",
    gradient: "from-azure/30 to-abyss",
    bgGlow: "oklch(0.7 0.18 240 / 40%)",
  },
  action: {
    label: variant.theme.labels.action,
    color: "text-amber-eclipse",
    gradient: "from-amber-eclipse/30 to-abyss",
    bgGlow: "oklch(0.78 0.16 60 / 40%)",
  },
  modifier: {
    label: variant.theme.labels.modifier,
    color: "text-gold",
    gradient: "from-gold/20 to-abyss",
...
function ComboGame() {
  ...
  useEffect(() => {
    ...
    if (phase === "result") {
      if (store.lastResult && store.lastResult.scored.score >= store.highScore)
        sounds.play("record");
  ...
  return (
    <div className="relative h-[100dvh] w-screen overflow-hidden bg-abyss flex items-center justify-center font-serif select-none">
      ...
      <AnimatePresence>{isShowingAd && <AdBreakOverlay />}</AnimatePresence>
        ...
        <header className="relative z-20 flex items-center justify-between h-12">
          <div className="flex items-center gap-3">
            <motion.div
              ...
              className="p-2 rounded-full bg-card/20 ring-1 ring-gold/30 text-gold active:bg-card/40 transition-colors"
            >
            ...
            <div className="flex flex-col">
              ...
              <div className="w-20 h-1 bg-card/40 rounded-full mt-1 overflow-hidden">
                <motion.div
                  ...
                  initial={{ width: 0 }}
                  animate={{ width: `${(store.xp / getXpToNextLevel(store.level)) * 100}%` }}
                />
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {store.isDailyMode && (
              <div className="flex items-center gap-1 rounded-full bg-amber-eclipse/20 px-3 py-1 ring-1 ring-amber-eclipse/40">
                ...
                <span className="font-display text-[9px] text-amber-eclipse uppercase tracking-widest font-bold">
                  Daily
                </span>
              </div>
            )}
            <div className="flex items-center gap-2 rounded-2xl bg-card/30 px-3 py-2 ring-1 ring-gold/20 backdrop-blur-sm">
              ...
              <span className="font-display text-sm text-foreground font-bold">
                {store.isDailyMode ? store.dailyHighScore : store.highScore}
              </span>
            </div>
          </div>
        </header>
              ...
              >
                ...
                {activeEvent && (
                  <motion.div
                    ...
                    className={`w-full max-w-xs rounded-2xl ${UI_THEME.glass} p-3 text-center border-t-2 border-gold/40`}
                  >
                    <div
                      className={`font-display text-[10px] font-bold tracking-[0.3em] ${activeEvent.color}`}
                    >
                      {activeEvent.label}
                    </div>
                    <div className="text-[8px] text-muted-foreground uppercase tracking-widest mt-0.5">
                ...
                >
                  {drawnCards.map((card, i) => (
                    <DrawnCard key={card.id} card={card} index={i} variant={activeVariant} />
                  ))}
                </motion.div>
                ...
                {currentResult && (
                  <motion.div
                    ...
                    className="w-full max-w-xs mt-2 text-center space-y-4"
                  >
                    {/* Synergy Meter */}
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center px-1">
                        <span className={UI_THEME.text_caption}>Chiarezza Attuale</span>
                        <span className="text-[10px] font-display text-gold font-bold">
                          {currentResult.synergyLevel}%
                        </span>
                      </div>
                      <div className="h-2 w-full bg-card/60 rounded-full overflow-hidden ring-1 ring-gold/10">
                        <motion.div
                          ...
                          initial={{ width: 0 }}
                          animate={{ width: `${currentResult.synergyLevel}%` }}
                    ...
                    <div className="flex flex-col gap-1 items-center">
                      <h3
                        className={`${UI_THEME.text_h2} ${currentResult.stars >= 4 ? "gold-text" : "text-foreground"}`}
                      >
                        {currentResult.title}
                      </h3>
                      <div className="flex gap-2">
                        {currentResult.penalties.map((p) => (
                          <span
                            key={p}
                            className="text-[8px] font-bold text-rose uppercase tracking-widest px-2 py-0.5 bg-rose/10 rounded-full ring-1 ring-rose/30 animate-pulse"
                          >
                            {p}
                          </span>
                        ))}
                        {currentResult.bonuses.slice(0, 1).map((b) => (
                          <span
                            key={b}
                            className="text-[8px] font-bold text-gold uppercase tracking-widest px-2 py-0.5 bg-gold/10 rounded-full ring-1 ring-gold/30"
                          >
                            {b}
                          </span>
                        ))}
    ...
    >
      ...
      <div className="w-full max-w-xs space-y-4">
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={onPlay}
          className={`${UI_THEME.premium_btn} w-full py-5 gold-frame bg-gradient-to-r from-mystic to-mystic-glow text-foreground text-lg shadow-2xl`}
        >
          ENTRA NEL SOGNO
        </motion.button>
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          onClick={onDaily}
          className={`${UI_THEME.premium_btn} w-full py-3 border border-amber-eclipse/40 text-amber-eclipse text-[10px]`}
        >
          SFIDA QUOTIDIANA
        </motion.button>
      </div>
      ...
      <div className="mt-12 w-full max-w-[280px] p-4 rounded-3xl bg-card/20 ring-1 ring-gold/10 backdrop-blur-sm">
        <div className="flex items-center gap-2 mb-3 border-b border-gold/5 pb-2">
          ...
          <span className="text-[8px] uppercase tracking-widest text-gold/40">
            Sognatori Più Lucidi
          </span>
        </div>
        <div className="space-y-2">
          {getDailyLeaderboard(new Date().getDate())
            .slice(0, 3)
            .map((e, i) => (
    ...
    >
      ...
      <div className="flex-1 text-left min-w-0">
        <div className="flex items-center justify-between">
          <span className={`text-[8px] font-bold uppercase tracking-widest ${m.color}`}>
            {m.label}
          </span>
          <span className="text-[8px] font-display text-gold/60 capitalize">{card.rarity}</span>
        </div>
        <h4 className="font-display text-base text-foreground truncate tracking-wide mt-0.5">
          {card.name}
        </h4>
      </div>
    </motion.div>
...
function DrawingSlot({ index }: { index: number }) {
  return (
    <motion.div
      ...
      className="w-full max-w-xs h-20 rounded-2xl bg-card/20 border border-gold/10 flex items-center justify-center"
    >
    ...
    <div className="relative h-[100dvh] w-screen bg-abyss flex items-center justify-center p-6">
      ...
      <MobileFrame className="w-full max-w-md h-full bg-transparent flex flex-col items-center justify-center text-center gap-8 relative z-10">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="size-48 relative flex items-center justify-center rounded-full bg-card/20 ring-4 ring-gold/20 shadow-2xl backdrop-blur-md"
        >
          <span className={`font-display text-9xl font-bold ${color}`}>{rank}</span>
          <motion.div
            animate={{ scale: [1, 1.3, 1], opacity: [0, 0.4, 0] }}
            transition={{ repeat: Infinity, duration: 2 }}
            className="absolute -inset-8 rounded-full border-2 border-gold/20"
          />
        </motion.div>
        ...
        <div className="w-full max-w-xs grid grid-cols-2 gap-3">
          <div className="bg-card/20 p-4 rounded-3xl ring-1 ring-gold/10">
            <p className="text-2xl font-display text-gold">{result.scored.stars}</p>
            <p className={UI_THEME.text_caption}>Sincronia</p>
          </div>
          <div className="bg-card/20 p-4 rounded-3xl ring-1 ring-gold/10">
            <p className="text-2xl font-display text-mystic-glow">{result.rerollsUsed}</p>
            <p className={UI_THEME.text_caption}>Frammenti</p>
          </div>
        </div>
        ...
        <div className="w-full max-w-xs flex flex-col gap-4">
          <button
            onClick={onRestart}
            className={`${UI_THEME.premium_btn} py-5 bg-gold text-abyss font-bold shadow-xl`}
          >
            RIPROVA
          </button>
          <button onClick={onExit} className={UI_THEME.text_caption}>
            Torna alla Home
          </button>
        </div>
      </MobileFrame>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        animate={
          isExiting
            ? { scale: 10, opacity: 0, filter: "blur(8px)" }
      ...
      >
        ...
        <motion.div
          animate={{
            ...
            boxShadow:
              progress >= 100 ? "0 0 40px rgba(255, 215, 0, 0.4)" : "0 0 0px rgba(255, 215, 0, 0)",
          }}
          transition={{
            scale: {
              duration: progress >= 100 ? 0.6 : 4,
...
function DeckShuffle() {
  return (
    <div className="flex items-center justify-center h-full">
      <Loader2 className="h-12 w-12 text-gold animate-spin opacity-20" />
    </div>
  );
}
function AdBreakOverlay() {
  return (
    <div className="absolute inset-0 z-[100] bg-abyss/90 backdrop-blur-md flex items-center justify-center p-8 text-center text-gold font-display text-xl uppercase tracking-widest">
      Sincronizzazione...
    </div>
  );
}
