  ...
  return (
    <MobileFrame className="px-6 pt-10">
      ...
      >
        ...
        <h1
          className={`font-display text-5xl tracking-[0.3em] ${colors[result]} drop-shadow-[0_0_18px_var(--mystic-glow)]`}
        >
          {titles[result]}
        </h1>
      </motion.div>
        ...
        {TERRITORIES.map((t) => {
          ...
          return (
            <div key={t.id} className="rounded-xl gold-frame bg-card/60 p-3 text-center">
              ...
              <p
                className={`mt-2 font-display text-lg ${winT === "win" ? "text-gold" : winT === "lose" ? "text-rose" : "text-azure"}`}
              >
                {r.p} - {r.a}
              </p>
              <p className="text-[9px] uppercase text-muted-foreground">
                {winT === "win" ? "Tu vinci" : winT === "lose" ? "Avv. vince" : "Pareggio"}
              </p>
            </div>
          );
        })}
      ...
      <div className="mt-6 rounded-xl gold-frame bg-card/60 p-4">
        ...
        <div className="mt-3 flex items-center justify-around">
          <div className="flex items-center gap-1 text-sm">
            ...
            <span className="font-display">+{rewards.gold}</span>
          </div>
          <div className="flex items-center gap-1 text-sm">
            <Diamond className="h-4 w-4 text-mystic-glow" />{" "}
            <span className="font-display">+{rewards.gems}</span>
          </div>
          <div className="flex items-center gap-1 text-sm">
            <Package className="h-4 w-4 text-azure" />{" "}
            <span className="font-display">+{rewards.packs}</span>
          </div>
        </div>
        <p className="mt-2 text-center text-[10px] text-muted-foreground">
          Punti Ranked: {rewards.rankPointsDelta >= 0 ? "+" : ""}
          {rewards.rankPointsDelta}
        </p>
      </div>
