...
const TITLES = [
  {
    n: "Sognatore",
    lore: "Chi ha varcato la prima soglia.",
    req: "Completa il tutorial",
    unlocked: true,
  },
  {
    n: "Risvegliato",
    lore: "Coscienza che ha visto la verità.",
    req: "Vinci 10 partite",
    unlocked: true,
  },
  {
    n: "Coscienza Spezzata",
    lore: "Frantumato e ricomposto.",
    req: "Subisci 5 sconfitte di fila",
    unlocked: true,
  },
  {
    n: "Dominatore",
    lore: "Conquistatore di tutti i territori.",
    req: "Vinci 50 partite ranked",
    unlocked: false,
  },
  {
    n: "Eco Eterno",
    lore: "La cui voce risuona nei sogni altrui.",
    req: "Raggiungi rank Architetto",
    unlocked: false,
  },
  {
    n: "Architetto del Sogno",
    lore: "Plasmatore di realtà oniriche.",
    req: "Completa il Pass S3",
    unlocked: false,
  },
  {
    n: "Custode dei Ricordi",
    lore: "Guardiano della Memoria.",
    req: "Colleziona 18 carte rare",
    unlocked: false,
  },
  {
    n: "Tessitore di Maschere",
    lore: "Maestro dell'inganno.",
    req: "Gioca 100 Maschere",
    unlocked: false,
  },
];
...
function Titles() {
  ...
  const { play } = useSound();
  const equip = (t: string) => {
    play("lock");
    setEquipped(t);
    toast.success(`Titolo equipaggiato: ${t}`);
  };
  ...
  return (
    <MobileFrame>
      ...
      <p className="mt-3 text-center text-[10px] uppercase tracking-widest text-muted-foreground">
        Sbloccati: {TITLES.filter((t) => t.unlocked).length}/{TITLES.length}
      </p>
      ...
      <div className="mt-4 space-y-2 px-4 pb-6 overflow-y-auto">
        {TITLES.map((t) => {
          ...
          return (
            ...
            >
              <div
                className={`flex size-10 items-center justify-center rounded-lg ${isEq ? "bg-gold/30" : "bg-mystic/20"}`}
              >
                {!t.unlocked ? (
                  <Lock className="h-4 w-4 text-muted-foreground" />
                ) : (
                  <Award className={`h-5 w-5 ${isEq ? "text-gold" : "text-mystic-glow"}`} />
                )}
              </div>
