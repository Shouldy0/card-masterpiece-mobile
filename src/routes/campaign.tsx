...
const CAMPAIGN_NODES = [
  { id: 1, title: "Il Primo Risveglio", lore: "Ti ritrovi in un prato di memorie dimenticate. Un'ombra ti osserva.", reward: "Carta Rara" },
  { id: 2, title: "L'Eco del Trauma", lore: "I ricordi si fanno oscuri. Il dolore del passato prende forma.", reward: "100 Frammenti" },
  { id: 3, title: "Il Labirinto Onirico", lore: "Le regole della realtà si sfaldano. Nulla è come sembra.", reward: "Card Back Esclusivo" },
  { id: 4, title: "Il Guardiano del Varco", lore: "L'ultimo ostacolo tra te e la verità suprema della Reverie.", reward: "Titolo Leggendario" },
];
            ...
            return (
              ...
              >
                ...
                <div className={cn(
                  ...
                  isCurrent ? "bg-gold border-white scale-110 z-10" : 
                  isLocked ? "bg-abyss border-white/10" : "bg-mystic/40 border-gold/50"
                )}>
                  {isLocked ? <Lock className="size-4 text-white/30" /> : <Sparkles className={cn("size-5", isCurrent ? "text-black animate-pulse" : "text-gold")} />}
                </div>

                <div className={cn(
                  "p-5 rounded-[2rem] border backdrop-blur-md transition-all",
                  isCurrent ? "bg-gold/10 border-gold/40 shadow-[0_0_30px_rgba(255,215,0,0.1)]" : "bg-card/40 border-white/5"
                )}>
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="font-display text-sm font-bold text-white tracking-wider">{node.title}</h3>
                    <span className="text-[8px] uppercase tracking-widest text-gold/60 font-black">Nodo 0{node.id}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[8px] text-white/30 uppercase tracking-widest">Premio: {node.reward}</span>
                    </div>
