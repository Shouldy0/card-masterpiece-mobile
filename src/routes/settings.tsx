  ...
  return (
    <MobileFrame>
      ...
      <div className="mt-4 flex gap-2 px-4">
        {tabs.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 rounded-full py-1.5 text-[10px] uppercase tracking-widest ring-1 ${tab === t ? "bg-mystic/50 text-foreground ring-gold" : "bg-card/40 text-muted-foreground ring-gold/20"}`}
          >
            {t}
          </button>
        ))}
      ...
      <div className="mt-4 flex-1 space-y-1 px-4">
        {tab === "Gioco" && (
          <>
            <Slider
              ...
              onChange={(v: number) => toggleSetting("animSpeed", v)}
            />
            <Toggle label="Mostra Effetti" value={true} onChange={() => {}} />
            <Toggle
              label="Suggerimenti"
              value={settings.hints}
              onChange={(v: boolean) => toggleSetting("hints", v)}
            />
        ...
        {tab === "Account" && (
          <>
            ...
            <Link
              to="/connection"
              className="block rounded-md gold-frame bg-card/60 px-3 py-3 text-center text-xs"
            >
              Stato Connessione
            </Link>
          </>
      ...
      <div className="grid grid-cols-2 gap-2 p-4">
        <button className="rounded-full gold-frame bg-card/60 py-2.5 text-xs uppercase tracking-widest text-muted-foreground">
          Assistenza
        </button>
        <button
          onClick={handleLogout}
          className="rounded-full gold-frame bg-card/60 py-2.5 text-xs uppercase tracking-widest text-rose"
        >
          Esci
        </button>
      </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-md gold-frame bg-card/40 px-3 py-3">
      ...
      <button
        ...
        className={`flex h-5 w-10 items-center rounded-full p-0.5 transition-colors ${value ? "bg-mystic" : "bg-abyss"}`}
      >
        <span
          className={`size-4 rounded-full bg-foreground transition-transform ${value ? "translate-x-5" : ""}`}
        />
      </button>
    </div>
  );
