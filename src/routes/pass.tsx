...
const REWARDS = Array.from({ length: 30 }, (_, i) => {
  ...
  const free =
    tier % 5 === 0
      ? { kind: "pack", amount: 1, icon: Package }
      : tier % 3 === 0
        ? { kind: "gold", amount: 50, icon: Coins }
        : { kind: "frag", amount: 5, icon: Sparkles };
  const prem =
    tier % 10 === 0
      ? { kind: "legendary", amount: 1, icon: Crown }
      : tier % 5 === 0
        ? { kind: "gems", amount: 50, icon: Diamond }
        : tier % 2 === 0
          ? { kind: "gold", amount: 100, icon: Coins }
          : { kind: "pack", amount: 1, icon: Package };
          ...
          <div className="relative h-28 bg-gradient-to-br from-mystic via-mystic-glow/40 to-abyss p-4">
            ...
            <button
              onClick={() => {
                play("record");
                activatePremiumPass();
                toast.success("Pass Premium attivato");
              }}
              disabled={premium}
              className="absolute right-3 top-3 rounded-full gold-frame bg-gold/30 px-3 py-1 text-[10px] font-display uppercase disabled:opacity-50"
            >
              {premium ? "Attivo ✓" : "Acquista Pass"}
            </button>
          </div>
        </div>
          ...
          {REWARDS.map(({ tier, free, prem }) => {
            ...
            return (
              <div key={tier} className="grid grid-cols-[2.5rem_1fr_1fr] items-center gap-2">
                <div
                  className={`flex size-9 items-center justify-center rounded-md gold-frame font-display text-sm ${unlockedFree ? "bg-mystic/40 text-gold" : "bg-card/60 text-muted-foreground"}`}
                >
                  {tier}
                </div>
                <Tier
                  ...
                  onClaim={() =>
                    claim(
                      fKey,
                      `+${free.amount} ${free.kind}`,
                      free.kind === "gold" ? free.amount : 0,
                    )
                  }
                />
                <Tier
                  ...
                  onClaim={() =>
                    claim(
                      pKey,
                      `+${prem.amount} ${prem.kind}`,
                      prem.kind === "gold" ? prem.amount : 0,
                    )
                  }
                />
              </div>
  ...
  return (
    ...
    >
      ...
      {claimed ? (
        ...
        <span className="text-[9px] uppercase tracking-widest text-gold">Claim</span>
      ) : (
